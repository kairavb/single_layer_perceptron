...
def make_state(n_classes, resize, color, activation, captures=None):
    return {
        "n_classes": n_classes,
        "captures": captures or {i: [] for i in range(n_classes)},
        "resize": resize,
        "color": color,
        "activation": activation,
        "weights": init_weights(resize, color, n_classes),
        "trained": False,
    }


def parse_settings(size_str, color_mode, activation):
    return int(size_str), color_mode == "Color", activation
...
def ui_from_state(state):
    n = state["n_classes"]
    return state, summary(state), f"{n} classes", UNTRAINED_MSG, n


def apply_settings(size_str, color_mode, activation, state):
    n = state["n_classes"]
    resize, color, act = parse_settings(size_str, color_mode, activation)
    return ui_from_state(make_state(n, resize, color, act))
...
def change_classes(state, delta, size_str, color_mode, activation):
    ...
    if not MIN_CLASSES <= new_n <= MAX_CLASSES:
        return ui_from_state(state)
    resize, color, act = parse_settings(size_str, color_mode, activation)
    captures = {i: list(state["captures"].get(i, [])) for i in range(new_n)}
    return ui_from_state(make_state(new_n, resize, color, act, captures))


def on_frame(state, frame):
    if frame is None or not state["trained"]:
        return None if frame is None else frame, gr.update()
    act = ACTIVATIONS[state["activation"]]
    raw = predict_scores(frame, state["weights"], act, state["resize"], state["color"])
...
def train(state):
    ...
    act = ACTIVATIONS[state["activation"]]
    state["weights"] = train_weights(samples, state["weights"], BIAS, act, state["resize"], state["color"])
    state["trained"] = True
    state["captures"] = {i: [] for i in range(state["n_classes"])}
...
def show_weights(state):
    if not state["trained"]:
        return []
    return [(img, label(i)) for i, img in enumerate(weights_to_images(state["weights"], state["resize"], state["color"]))]
...
def build_ui():
    ...
    with gr.Blocks(title="Single Layer Perceptron") as demo:
        ...
        n_classes = gr.State(DEFAULT_CLASSES)
        ...
        with gr.Row(equal_height=False):
            ...
            with gr.Column(scale=5, elem_classes=["right-panel"]):
                ...
                with gr.Group():
                    ...
                    @gr.render(inputs=[n_classes])
                    def capture_buttons(n):
                        with gr.Row(elem_classes=["capture-grid"]):
                            for i in range(int(n)):
                                btn = gr.Button(label(i), min_width=64, scale=0)
        ...
        webcam.stream(on_frame, [state, webcam], [latest_frame, classify], stream_every=STREAM_EVERY, show_progress="hidden")
        restart_btn.click(None, None, None, js="() => { window.location.reload(); }")

        settings_in = [size_dd, color_dd, activation_dd, state]
        update_out = [state, counts, class_count, classify, n_classes]
        class_in = [state, size_dd, color_dd, activation_dd]
        add_btn.click(lambda s, sz, c, a: change_classes(s, 1, sz, c, a), class_in, update_out)
        remove_btn.click(lambda s, sz, c, a: change_classes(s, -1, sz, c, a), class_in, update_out)
        for dd in (activation_dd, color_dd, size_dd):
            dd.change(apply_settings, settings_in, update_out)
